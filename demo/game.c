...
void wall_init(state_t *state) {
  ...
  for (size_t i = 0; i < NUM_LEVELS; i++){
    ...
    create_collision(scene, right_wall, state -> user_body, sticky_collision, (char*)"v_0", WALL_ELASTICITY);
    create_collision(scene, left_wall, state -> user_body, sticky_collision, (char*)"v_0", WALL_ELASTICITY);
  ...
  create_collision(scene, platform, state -> user_body, sticky_collision, (char*)"v_0", WALL_ELASTICITY);
