...
bool emscripten_main(state_t *state) {
  ...
  if (state->vertical_offset >= 2800 && state->distance_based_message_tracker_halfpoint == false) {
    printf("%s", PORTAL_SENSED_MESSAGE);
    state->distance_based_message_tracker_halfpoint = true;
  } else if (state->vertical_offset >= 5500 && state->distance_based_message_tracker_portal == false) {
